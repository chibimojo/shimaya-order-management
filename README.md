...
gas/
  ...
  sheetManager.gs    ヘッダー名ベースのスプレッドシート読み書きユーティリティ（汎用）
  orderManagement.gs 受注管理のビジネスロジック（検索・採番・登録）
  doGet.gs           Web アプリのエントリーポイント
...
で読み書きする。`orderManagement.gs` は `sheetManager.gs` の汎用関数
（`getSheet_` / `getAllRecords_` / `filterRecordsByColumn_` / `appendRecord_`）を
ラップして画面向けの関数を提供する。
...
| 商品ID | 学校 | 商品名 | サイズ | 色 | 価格 | 裾上げ要否 |
| --- | --- | --- | --- | --- | --- | --- |

- 「裾上げ要否」が `Y` の商品（スラックス等）は、受注登録画面で
  「裾上げ総丈」入力欄が自動的に表示される。
...
| 注文番号 | 受注日 | 子どもID | 商品ID | 数量 | 裾上げ総丈 | 特記事項 |
| --- | --- | --- | --- | --- | --- | --- |

注文番号は `#00001` 形式で自動採番される（既存の最大番号 + 1）。
`LockService` により、複数スタッフが同時に登録しても番号が重複しないようにしている。
...
2. 電話番号を入力して「検索」→ 保護者情報と子ども一覧が表示される。
3. 子どもを選択して「次へ」→ その子どもの在籍校向けの商品一覧が表示される。
4. 商品を選択 → 数量を入力（裾上げ要否「Y」の商品は「裾上げ総丈」も入力）。
5. 「注文を保存」を押すと受注シートに保存され、注文番号が表示される。
